import cv2
import numpy as np
from main import HybridizedAlgorithm

# import types
# from google.colab.patches import cv2_imshow
hybrid_algorithm = HybridizedAlgorithm()

def msg_to_bin(msg):  # converting types to binary
    if type(msg) == str:
...
def hide_data(img, secret_msg):  # defining function to hide the secret message into the image
    ...
    if len(secret_msg) > nBytes:
        raise ValueError("Error encountered in insufficient bytes, need bigger image or less data!!")
    ...
    for values in img:
        for pixels in values:
            ...
            if dataIndex < dataLen:
                ...
                pixels[1] = int(r[:-1] + bin_secret_msg[dataIndex], 2)
                dataIndex += 1
            if dataIndex < dataLen:
                # hiding the data into LSB of Blue pixel
                pixels[2] = int(r[:-1] + bin_secret_msg[dataIndex], 2)
                dataIndex += 1
...
def show_data(img):  # Decoding the hidden message from the Steganographic image
    bin_data = ""
    for values in img:
        for pixels in values:
...
def encodeText(data):  # defining the function to encode data into image
    ...
    print("The shape of the image is: ", img.shape)  # checking the image shape to calculate the number
    print("The original image is as shown below: ")
    # resizing the image as per the need
    resizedImg = cv2.resize(img, (500, 500))
    # displaying the image
    cv2.imshow("5", resizedImg)  # cv2.imshow(resizedImg)
...
# defining the function to decode data into image
...
def decodeText():
    ...
    img = cv2.imread(img_name)  # reading the image using imread() function

    print("The Steganographic image is as follow: ")
    resizedImg = cv2.resize(img, (500, 500))  # resizing the actual image as per the needs
    cv2.imshow("1", resizedImg)  # cv2.imshow(resizedImg) #displaying the steganographic image
...
def run_hybrid(hybrid_algorithm):
    # Create hybrid algorithm object
...
def steganography():  # image steganography
    print('''
        1.) Encrypt First
        2.) Decrypt First
    ''')

    start_choice = input("Enter start choice number: ")
    if start_choice == "1":
        crypto_objects = run_hybrid(hybrid_algorithm)

        n = int(input("Image steganography \n1. Encode the data \n2. Decode the data \nSelect the option: "))
        # if(n == 1):
        print("\nEncoding...")
        encodeText(data=crypto_objects.get("encrypted_data"))
    elif start_choice == "2":
        # elif(n == 2):
        print("\nDecoding...")
        # print("Decoded message is " + decodeText())
        decoded_image = decodeText()
        # decrypted_text = hybrid_algorithm.decrypt_data(decoded_image)
        print(decoded_image)
